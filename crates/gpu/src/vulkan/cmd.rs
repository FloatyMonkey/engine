...
impl gpu::CmdListImpl<Device> for CmdList {
	...
	fn barriers(&self, barriers: &gpu::Barriers<Device>) {
		let memory_barriers = barriers
			.global
			.iter()
			.map(|barrier| vk::MemoryBarrier2::default())
			.collect::<Vec<_>>();

		let buffer_memory_barriers = barriers
			.buffer
			.iter()
			.map(|barrier| vk::BufferMemoryBarrier2::default().buffer(barrier.buffer.buffer))
			.collect::<Vec<_>>();

		let image_memory_barriers = barriers
			.texture
			.iter()
			.map(|barrier| {
	...
	fn set_blend_constant(&self, color: gpu::Color<f32>) {
		unsafe {
			self.device
				.cmd_set_blend_constants(self.cmd(), &[color.r, color.g, color.b, color.a]);
		}
	}
	...
	fn set_index_buffer(&self, buffer: &Buffer, offset: u64, format: gpu::Format) {
		...
		unsafe {
			self.device
				.cmd_bind_index_buffer(self.cmd(), buffer.buffer, offset, vk_format);
		}
	}
	...
	fn dispatch_rays(&self, desc: &gpu::DispatchRaysDesc) {
		unsafe {
			self.ray_tracing_pipeline_ext.cmd_trace_rays(
				self.cmd(),
				&desc.raygen.as_ref().map_or(Default::default(), |t| {
				...
				&desc.miss.as_ref().map_or(Default::default(), |t| {
					vk::StridedDeviceAddressRegionKHR {
						device_address: t.ptr.0,
						stride: t.stride as _,
						size: t.size as _,
				...
				&desc.hit_group.as_ref().map_or(Default::default(), |t| {
					vk::StridedDeviceAddressRegionKHR {
						device_address: t.ptr.0,
						stride: t.stride as _,
						size: t.size as _,
				...
				&desc.callable.as_ref().map_or(Default::default(), |t| {
					vk::StridedDeviceAddressRegionKHR {
						device_address: t.ptr.0,
						stride: t.stride as _,
						size: t.size as _,
	...
	fn build_acceleration_structure(&self, desc: &gpu::AccelerationStructureBuildDesc<Device>) {
		...
		unsafe {
			...
			self.acceleration_structure_ext
				.cmd_build_acceleration_structures(self.cmd(), &infos, build_range_infos);
		}
	}
