...
use math::{Mat4, Quaternion, Unit, UnitQuaternion, Vec3, transform::Transform3};
...
fn navigation_gizmo(ui: &mut egui::Ui, p: egui::Pos2, rotation: UnitQuaternion<f32>) {
	...
	let mut axes = [
		(
			"X",
			(rotation * Vec3::X).cmul(scale),
			egui::Color32::from_rgb(246, 54, 82),
		),
		(
			"Y",
			(rotation * Vec3::Y).cmul(scale),
			egui::Color32::from_rgb(112, 164, 28),
		),
		(
			"Z",
			(rotation * Vec3::Z).cmul(scale),
			egui::Color32::from_rgb(47, 132, 227),
		),
		(
			"-X",
			(rotation * -Vec3::X).cmul(scale),
			egui::Color32::from_rgba_unmultiplied(246, 54, 82, 128),
		),
		(
			"-Y",
			(rotation * -Vec3::Y).cmul(scale),
			egui::Color32::from_rgba_unmultiplied(112, 164, 28, 128),
		),
		(
			"-Z",
			(rotation * -Vec3::Z).cmul(scale),
			egui::Color32::from_rgba_unmultiplied(47, 132, 227, 128),
		),
	];
	...
	fn ui(&mut self, ui: &mut egui::Ui, ctx: &mut MyContext) {
		...
		let response = ui.add(
			egui::Image::from_texture((
				egui::TextureId::User(ctx.viewport_texture_srv as u64),
				max_rect.size(),
			))
		...
		let mut toolbar_ui = ui.child_ui_with_id_source(
			egui::Align2::LEFT_TOP
				.align_size_within_rect(egui::vec2(32.0, 400.0), max_rect.shrink(10.0)),
		...
		egui::Frame::none().show(&mut toolbar_ui, |ui| {
			...
			visuals.widgets.inactive.weak_bg_fill =
				egui::Color32::from_rgba_premultiplied(10, 11, 12, 210);
			...
			if ui
				.add(
					egui::ImageButton::new(egui::include_image!(
						"../../resources/mouse-pointer.svg"
					))
					.selected(self.gizmo_mode.is_none())
					.rounding(egui::Rounding {
						nw: 5.0,
						ne: 5.0,
						se: 0.0,
			...
			if ui
				.add(
					egui::ImageButton::new(egui::include_image!("../../resources/move-3d.svg"))
						.selected(self.gizmo_mode == Some(egui_gizmo::GizmoMode::Translate)),
				)
				.clicked()
			{
			...
			if ui
				.add(
					egui::ImageButton::new(egui::include_image!("../../resources/rotate-3d.svg"))
						.selected(self.gizmo_mode == Some(egui_gizmo::GizmoMode::Rotate)),
				)
				.clicked()
			{
			...
			if ui
				.add(
					egui::ImageButton::new(egui::include_image!("../../resources/scale-3d.svg"))
						.selected(self.gizmo_mode == Some(egui_gizmo::GizmoMode::Scale))
						.rounding(egui::Rounding {
							nw: 0.0,
							ne: 0.0,
							se: 5.0,
		...
		ui.painter().text(
			cursor.left_top() + egui::vec2(55.0, 10.0),
			egui::Align2::LEFT_TOP,
			format!("dt: {:.2} ms", self.frame_time * 1000.0),
			egui::FontId::monospace(12.0),
			egui::Color32::WHITE,
		);

		let (camera_transform, camera) = ctx
			.world
		...
		navigation_gizmo(
			ui,
			cursor.right_top() + egui::vec2(-10.0 - 80.0, 10.0),
			camera_transform.rotation.inv(),
		);
		...
		if let Some(selection) = ctx.selection.iter().next()
			&& let (Some(mode), Some(transform)) = (
				self.gizmo_mode,
				ctx.world.entity_mut(*selection).get_mut::<Transform3>(),
			) {
		...
		if let Some((camera_transform, _)) = ctx
			.world
			.query::<(&mut Transform3, &Camera)>()
			.iter()
			.next()
		{
