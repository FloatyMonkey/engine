...
use windows::{
	Win32::Foundation::*,
	Win32::Graphics::{Direct3D::*, Direct3D12::*, Dxgi::Common::*, Dxgi::*},
...
fn map_buffer_usage_flags(usage: super::BufferUsage) -> D3D12_RESOURCE_FLAGS {
	...
	if usage.contains(super::BufferUsage::UNORDERED_ACCESS) {
		dx_flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
	}
	if usage.contains(super::BufferUsage::ACCELERATION_STRUCTURE) {
		dx_flags |= D3D12_RESOURCE_FLAG_RAYTRACING_ACCELERATION_STRUCTURE;
	}
...
fn map_texture_usage_flags(usage: super::TextureUsage) -> D3D12_RESOURCE_FLAGS {
	...
	if usage.contains(super::TextureUsage::RENDER_TARGET) {
		dx_flags |= D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
	}
	if usage.contains(super::TextureUsage::DEPTH_STENCIL) {
		dx_flags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
	}
...
fn map_color_attachments(
	attachments: &[super::ColorAttachment],
) -> [D3D12_RENDER_TARGET_BLEND_DESC; 8] {
	let mut dx_descs: [D3D12_RENDER_TARGET_BLEND_DESC; 8] =
...
fn map_acceleration_structure_build_flags(
...
fn map_acceleration_structure_geometry_flags(
	...
	if flags.contains(super::AccelerationStructureGeometryFlags::OPAQUE) {
		dx_flags |= D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE;
	}
	if flags.contains(super::AccelerationStructureGeometryFlags::NO_DUPLICATE_ANY_HIT_INVOCATION) {
		dx_flags |= D3D12_RAYTRACING_GEOMETRY_FLAG_NO_DUPLICATE_ANYHIT_INVOCATION;
	}
...
fn map_acceleration_structure_instance_flags(
	...
	if flags.contains(super::AccelerationStructureInstanceFlags::TRIANGLE_CULL_DISABLE) {
		dx_flags |= D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_CULL_DISABLE;
	}
	if flags.contains(super::AccelerationStructureInstanceFlags::TRIANGLE_FRONT_CCW) {
		dx_flags |= D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_FRONT_COUNTERCLOCKWISE;
	}
	if flags.contains(super::AccelerationStructureInstanceFlags::FORCE_OPAQUE) {
		dx_flags |= D3D12_RAYTRACING_INSTANCE_FLAG_FORCE_OPAQUE;
	}
	if flags.contains(super::AccelerationStructureInstanceFlags::FORCE_NON_OPAQUE) {
		dx_flags |= D3D12_RAYTRACING_INSTANCE_FLAG_FORCE_NON_OPAQUE;
	}

	dx_flags
}

fn map_load_op<T: Default>(
	load_op: super::LoadOp<T>,
) -> (D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE, T) {
	match load_op {
		super::LoadOp::Load => (
...
fn get_adapter(
	...
	unsafe {
		...
		let adapter = factory
			.EnumAdapterByGpuPreference::<IDXGIAdapter1>(adapter_index as u32, gpu_preference)?;
		let desc = adapter.GetDesc1()?;
...
fn create_swap_chain_rtv(
	...
	unsafe {
		...
		for i in 0..num_bb {
			...
			device
				.device
				.CreateRenderTargetView(&render_target, None, h);
...
impl Heap {
	fn new(
		...
		let heap: ID3D12DescriptorHeap =
			unsafe { device.CreateDescriptorHeap(&heap_desc).unwrap() };
	...
	fn create_root_signature(
			...
			for binding in bindings {
				let range = D3D12_DESCRIPTOR_RANGE1 {
					...
					Flags: D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE
						| D3D12_DESCRIPTOR_RANGE_FLAG_DATA_VOLATILE, // TODO: Optimization potential?
					OffsetInDescriptorsFromTableStart: binding
						.offset
						.unwrap_or(D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND),
				};
		...
		if let Some(samplers) = &layout.static_samplers {
			for sampler in samplers {
				static_samplers.push(D3D12_STATIC_SAMPLER_DESC {
					...
					BorderColor: sampler.sampler_desc.border_color.map_or(
						D3D12_STATIC_BORDER_COLOR_TRANSPARENT_BLACK,
						map_static_border_color,
					),
		...
		unsafe {
			...
			if let Some(blob) = error {
				let error = String::from_raw_parts(
			...
			let slice =
				std::slice::from_raw_parts(sig.GetBufferPointer() as *mut u8, sig.GetBufferSize());
...
impl super::DeviceImpl for Device {
	...
	fn new(desc: &super::DeviceDesc) -> Device {
		unsafe {
			...
			let (adapter, adapter_info) =
				get_adapter(&dxgi_factory, map_power_preference(desc.power_preference)).unwrap();
			...
			let capabilities = super::Capabilities {
				raytracing: res.is_ok()
					&& feature_options5.RaytracingTier == D3D12_RAYTRACING_TIER_1_1,
			};
			...
			let resource_heap =
				Heap::new(&device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1_000_000); // Tier 1 limit
			...
			let pix = desc
				.validation
				.contains(super::Validation::DEBUGGER)
				.then(pix::WinPixEventRuntime::load)
				.flatten();
	...
	) -> result::Result<Surface, super::Error> {
		unsafe {
			...
			let swap_chain1 = self.dxgi_factory.CreateSwapChainForHwnd(
				&self.command_queue,
	...
	fn create_buffer(&mut self, desc: &super::BufferDesc) -> result::Result<Buffer, super::Error> {
		unsafe {
			...
			let srv_index = desc
				.usage
				.contains(super::BufferUsage::SHADER_RESOURCE)
				.then(|| {
			...
			let uav_index = desc
				.usage
				.contains(super::BufferUsage::UNORDERED_ACCESS)
				.then(|| {
					let h = self.resource_heap.allocate();
			...
			let cpu_ptr = {
				...
				match desc.memory {
					...
					super::Memory::CpuToGpu => resource
		...
		unsafe {
			...
			self.device.CreateCommittedResource3(
				...
				&D3D12_RESOURCE_DESC1 {
					...
					DepthOrArraySize: if desc.depth > 1 {
						desc.depth as u16
					} else {
						desc.array_size as u16
					},
			...
			let srv_index = desc
				.usage
				.contains(super::TextureUsage::SHADER_RESOURCE)
				.then(|| {
					let h = self.resource_heap.allocate();
					self.device.CreateShaderResourceView(
						&resource,
			...
			let uav_index = desc
				.usage
				.contains(super::TextureUsage::UNORDERED_ACCESS)
				.then(|| {
					let h = self.resource_heap.allocate();
					self.device
						.CreateUnorderedAccessView(&resource, None, None, h);
					self.resource_heap.handle_to_index(&h)
				});
	...
	fn create_sampler(
		&mut self,
		desc: &super::SamplerDesc,
	) -> result::Result<Self::Sampler, super::Error> {
	...
	) -> result::Result<AccelerationStructure, super::Error> {
						...
						Anonymous: D3D12_SHADER_RESOURCE_VIEW_DESC_0 {
							RaytracingAccelerationStructure:
								D3D12_RAYTRACING_ACCELERATION_STRUCTURE_SRV {
									Location: desc.buffer.resource.GetGPUVirtualAddress()
										+ (desc.offset as u64),
								},
						},
						Shader4ComponentMapping: D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING,
	...
	fn create_graphics_pipeline(
	...
	) -> result::Result<GraphicsPipeline, super::Error> {
		...
		let mut dx_desc = D3D12_GRAPHICS_PIPELINE_STATE_DESC {
			...
			VS: desc.vs.map_or(D3D12_SHADER_BYTECODE::default(), |vs| {
				D3D12_SHADER_BYTECODE {
					pShaderBytecode: vs.as_ptr() as _,
					BytecodeLength: vs.len(),
				}
			}),
			PS: desc.ps.map_or(D3D12_SHADER_BYTECODE::default(), |ps| {
				D3D12_SHADER_BYTECODE {
			...
			DepthStencilState: D3D12_DEPTH_STENCIL_DESC {
				...
				DepthWriteMask: if depth_stencil.depth_write_enable {
	...
	fn create_raytracing_pipeline(
	...
	) -> result::Result<Self::RaytracingPipeline, super::Error> {
		...
		for group in &desc.groups {
			...
			groups.push(D3D12_HIT_GROUP_DESC {
				...
				AnyHitShaderImport: group
					.any_hit
					.map_or(PCWSTR::null(), |i| exports[i as usize].Name),
				ClosestHitShaderImport: group
					.closest_hit
					.map_or(PCWSTR::null(), |i| exports[i as usize].Name),
				IntersectionShaderImport: group
					.intersection
					.map_or(PCWSTR::null(), |i| exports[i as usize].Name),
			});
	...
	fn create_texture_view(
		...
		unsafe {
			...
			self.device
				.CreateUnorderedAccessView(&texture.resource, None, Some(&desc), h);
		}
	...
	fn acceleration_structure_sizes(
		...
		unsafe {
			self.device
				.GetRaytracingAccelerationStructurePrebuildInfo(&info.desc, &mut prebuild_info);
		}
		...
		unsafe {
			...
			if fv != 0 {
				...
				WaitForMultipleObjects(
					&[
						self.swap_chain.GetFrameLatencyWaitableObject(),
						self.fence_event,
					],
					true,
					INFINITE,
...
impl super::SurfaceImpl<Device> for Surface {
	...
	fn update(&mut self, device: &mut Device, size: [u32; 2]) {
		...
		self.backbuffer_textures =
			create_swap_chain_rtv(&self.swap_chain, device, self.num_buffers);
	...
	fn present(&mut self, device: &Device) {
		...
		unsafe { self.swap_chain.Present(sync, flags) }
			.ok()
			.unwrap();
...
impl super::AccelerationStructureImpl<Device> for AccelerationStructure {
	...
	fn write_instance_descriptor(
		...
		let dx_instance = D3D12_RAYTRACING_INSTANCE_DESC {
			...
			_bitfield2: (instance.contribution_to_hit_group_index & 0xffffff)
				| ((map_acceleration_structure_instance_flags(instance.flags).0 as u32) << 24),
			AccelerationStructure: instance.bottom_level.0,
		};

		unsafe {
			std::ptr::copy_nonoverlapping(
				&dx_instance as *const _ as _,
				slice.as_mut_ptr(),
				size_of::<D3D12_RAYTRACING_INSTANCE_DESC>(),
			);
		}
	...
	fn write_shader_identifier(&self, group_index: usize, slice: &mut [u8]) {
		unsafe {
			std::ptr::copy_nonoverlapping(
				self.identifiers
					.as_ptr()
					.add(group_index * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES as usize),
				slice.as_mut_ptr(),
				D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES as usize,
			);
		}
...
impl AccelerationStructureInfo {
	...
	fn map_triangles(
		triangles: &super::AccelerationStructureTriangles,
	) -> D3D12_RAYTRACING_GEOMETRY_DESC {
		D3D12_RAYTRACING_GEOMETRY_DESC {
	...
	fn build(inputs: &super::AccelerationStructureBuildInputs) -> Self {
		let geometries: Vec<D3D12_RAYTRACING_GEOMETRY_DESC> = match &inputs.entries {
			super::AccelerationStructureEntries::AABBs(aabbs) => {
				aabbs.iter().map(Self::map_aabbs).collect()
			}
			super::AccelerationStructureEntries::Triangles(triangles) => {
				triangles.iter().map(Self::map_triangles).collect()
			}
